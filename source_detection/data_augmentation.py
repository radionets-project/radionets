...
def rough_gaussian_noise(image, strength = 0.05):
    pixel = image.shape[1]
    bundle_size = 1
    x = np.linspace(0, (pixel/2) - 1, num=pixel//12)
    y = np.linspace(0, (pixel/2) - 1, num=pixel//12)
    X, Y = np.meshgrid(x, y)
    strength = np.random.uniform(0.02,0.05)
    grid = np.array([np.random.normal(0,strength,X.shape) , X, Y])
    grid = np.repeat(
        grid[None, :, :, :],
        bundle_size,
        axis=0,
    )
    dimg = torch.FloatTensor(grid[0][0])
    dimg = dimg.unsqueeze(0)
    dimg = dimg.unsqueeze(0)
    uimg = F.interpolate(dimg, size = (pixel,pixel) , mode = 'bilinear')
