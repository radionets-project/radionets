    ...
    for t in range(num_bundles):
        with h5py.File(path+str(t)+'.h5', "w") as hf:
            ...
            for r in range(bundle_size):
                ...
                num_objects = 200#np.random.randint(150,200)
                bboxes = np.zeros((num_objects,4))
                labels = np.zeros((num_objects,1))
                diffuse_limiter = 0
                if num_objects == 0:
                    ...
                    for i in range(num_objects): 
                        ...
                        if rand == 0:
                            ...
                            xmin = (c[0]-2*s[0]).clip(0,img_size*10)/(img_size*10)
                            ymin = (c[1]-2*s[1]).clip(0,img_size*10)/(img_size*10)
                            xmax = (c[0]+2*s[0]).clip(0,img_size*10)/(img_size*10)
                            ymax = (c[1]+2*s[1]).clip(0,img_size*10)/(img_size*10)
                            bbox = np.array([xmin.clip(0,1), ymin.clip(0,1), xmax.clip(0,1), ymax.clip(0,1)], dtype = float)
                            label = np.array([2]) #nodiff
                            bboxes[i] = bbox 
                            labels[i] = label
                            ...
                            xmin = (c[0]-(img_size/50+1)).clip(0,img_size*10)/(img_size*10)
                            ymin = (c[1]-(img_size/50+1)).clip(0,img_size*10)/(img_size*10)
                            xmax = (c[0]+(img_size/50+1)).clip(0,img_size*10)/(img_size*10)
                            ymax = (c[1]+(img_size/50+1)).clip(0,img_size*10)/(img_size*10)
                            bbox = np.array([xmin.clip(0,1), ymin.clip(0,1), xmax.clip(0,1), ymax.clip(0,1)], dtype = float)
                            label = np.array([3]) #nodiff
                            bboxes[i] = bbox 
                            labels[i] = label
        ...
        with h5py.File(path+str(t)+'.h5', "w") as hf:
            ...
            if num_objects == 0:
                ...
                while bundle < bundle_size:
                    ...
                    #final_image = rough_gaussian_noise(final_image) #add rough noise
...
def noisy_data_multiclass(img_size, mosaic_scale, bundle_size, num_bundles,path): #TEMP CHANGES: NO NOISE!
    for t in tqdm(range(num_bundles)):
        with h5py.File(path+str(t)+'.h5', "w") as hf:
            all_images = []
            all_bboxes = []
