...
class resnet_amp_no_symmetry(nn.Module):
    def __init__(self, img_size):
        super().__init__()
        self.conv1_amp = nn.Sequential(*conv_amp(1, 4, (23, 23), 1, 11, 1))
        self.conv2_amp = nn.Sequential(*conv_amp(4, 8, (21, 21), 1, 10, 1))
...
class resnet_phase(nn.Module):
    ...
    def forward(self, x):
        ...
        phase = x[:, 1, :].unsqueeze(1)
        inp_phase = inp[:, 1].unsqueeze(1)
        inp_phase_first = inp_phase.clone()
        ...
        phase = phase + inp_phase
        inp_phase = phase.clone()
        ...
        phase = phase + inp_phase + inp_phase_first
...
class resnet_phase_no_symmetry(nn.Module):
    def __init__(self, img_size):
        super().__init__()
        self.conv1_phase = nn.Sequential(
            *conv_phase(1, 4, (23, 23), 1, 11, 1, add=-2.1415)
