...
class superRes_resTest(nn.Module):
    def __init__(self, img_size):
        super().__init__()
        self.img_size = img_size

        self.conv1_amp = ResBlock_amp(1, 8)
        self.conv1_phase = ResBlock_phase(1, 8)

        self.conv2_amp = ResBlock_amp(8, 16)
        self.conv2_phase = ResBlock_phase(8, 16)

        self.conv3_amp = ResBlock_amp(16, 32)
        self.conv3_phase = ResBlock_phase(16, 32)

        self.conv4_amp = ResBlock_amp(32, 64)
        self.conv4_phase = ResBlock_phase(32, 64)

        self.conv5_amp = ResBlock_amp(64, 128)
        self.conv5_phase = ResBlock_phase(64, 128)

        self.final_amp = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(128, img_size ** 2)
        )
        self.final_phase = nn.Sequential(
            nn.AdaptiveAvgPool2d(1), nn.Flatten(), nn.Linear(128, img_size ** 2)
        )
...
class superRes_res18skip(nn.Module):  # ResNet18 with skip to LinLayer
    def __init__(self, img_size):
        super().__init__()
        torch.cuda.set_device(1)
        self.img_size = img_size
...
class superRes_res34(nn.Module):
    def __init__(self, img_size):
        ...
        self.layer1_phase = nn.Sequential(
            ResBlock_phase(64, 64), ResBlock_phase(64, 64), ResBlock_amp(64, 64)
        )
