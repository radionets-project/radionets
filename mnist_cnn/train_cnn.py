...
from training import view_tfm, sched_no, Recorder, AvgStatsCallback, ParamScheduler,\
                        BatchTransformXCallback, CudaCallback, Runner, SaveCallback
...
@click.option('-positive', type=bool, required=False)
...
def main(train_path, valid_path, model_path, num_epochs, lr, log=True, quantile=False, positive=False,
    ...
    train_ds, valid_ds = prepare_dataset(x_train, y_train, x_valid, y_valid,
                                         log=log, quantile=quantile, positive=positive)
